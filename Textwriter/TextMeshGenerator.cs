    ...
    public static TextVertex[] GenerateVertices(BuiltText builtText)
    {
        ...
        foreach (ShapedText text in builtText)
        {
            ...
            foreach (BuiltGlyph glyphInfo in glyphInfos)
            {
                ...
                {
                    float originX = advanceX + text.OffsetX;
                    float originY = advanceY + text.OffsetY;
                    
                    float minX = (originX + glyph.HorizontalBearingX + glyphInfo.OffsetX) / 64.0f;
                    float minY = (originY - glyph.HorizontalBearingY + glyph.Height + glyphInfo.OffsetY) / 64.0f;
                    float maxX = (originX + glyph.HorizontalBearingX + glyph.Width + glyphInfo.OffsetX) / 64.0f;
                    float maxY = (originY - glyph.HorizontalBearingY + glyphInfo.OffsetY) / 64.0f;
